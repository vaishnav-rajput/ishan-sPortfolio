...
const Card = ({number, title, description, side}) => {
  ...
  return (
    ...
    <motion.div  className={` ${side === "right" ? "lg:right-0 lg:translate-x-10" : "lg:left-[20%]"} transition-all duration-200 ease-in group projectCard lg:mt-12 sm:mt-4 xs:mt-3 project-card project-card-${number} relative shadow-custom lg:w-[75%] lg:h-[450px] xs:h-[250px] sm:h-[300px] sm:w-auto xs:w-auto bg-cover bg-center bg-no-repeat  overflow-visible `} id="project1">
        ...
        <motion.div className="project-content  h-full w-full    flex flex-col gap-y-5 justify-center items-center">
            <p className="project-subHeading xs:text-l lg:text-3xl text-white transition-all duration-200 ease-in  lg:font-semibold font-raleway z-30 group-hover:scale-125" >{description}
            </p>
            <h2 className="project-heading xs:text-l lg:text-2xl font-bold transition-all duration-200 ease-in text-white font-poppins overflow-hidden z-30 group-hover:scale-125 ">{title}</h2>
        </motion.div>
